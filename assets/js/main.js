...
function submitForm(){
	_("registerdo").disabled = true;
	_("status").innerHTML = 'even geduld ...';
	var formdata = new FormData();
	formdata.append( "u_name", _("u_name").value );
	formdata.append( "u_email", _("u_email").value );
	formdata.append( "u_message", _("u_message").value );
	var ajax = new XMLHttpRequest();
	ajax.open( "POST", "mail-it.php" );
	ajax.onreadystatechange = function() {
		if(ajax.readyState == 4 && ajax.status == 200) {
			if(ajax.responseText == "success"){
				_("ContactForm").innerHTML = '<h6>Bedankt '+_("u_name").value+', Je bericht is verzonden.</h6>';
			} else {
				_("status").innerHTML = ajax.responseText;
				_("registerdo").disabled = false;
			}
		}
	}
	ajax.send( formdata );
